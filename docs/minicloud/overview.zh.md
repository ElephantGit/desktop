...
- `ora-controller`：Controller 可执行程序本身提供过渡 clone API，没有独立的 minicloud server，
  见 [Controller 运行时](../controller/local-runtime.zh.md)。
- Node 保持独立，使用已有本机 IPC 及 host／guardian 管理的 Git 执行；开发时由 Controller 以
  `--single-node` 启动。

两个开发监听入口均只供本机访问，浏览器通过 Vite proxy 调用 Controller。
不增加认证、令牌、租户、额外安全体系或生产部署。
回环监听是部署约束，不承诺隔离恶意本机调用方。

Controller 仍拥有操作记录；其 API 层不另建任务数据库、不执行 Git。
...
页面刷新或 Controller 重启后读取原记录，不重新提交新执行。
