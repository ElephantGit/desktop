...
> 当前目标已调整为[clone 指定仓库与分支](minimal-loop.zh.md)。下文覆盖 Worktree 恢复与受管 clone，
> 不表示 Client 可观察的完整闭环已完成。

Linux `ora-node` 可执行程序持有显式配置的 Node 数据库，恢复待处理的 Worktree／clone 执行并处理正常停止。
...
独立进程测试前先构建 `cargo build -p ora-node -p ora-controller -p ora-process-host -p ora-process-guardian`。
