...
- [x] Deployment supplies `~/.ora/node`; database name is `ora-node.sqlite3` inside that directory. Tests inject temporary homes.
...
- [ ] Complete focused tests/lint and full `task test`, recording results and limitations.
...
| V6  | Stops after acceptance, Running and add recover only with sufficient evidence; branch-only effects stay Unknown                             |
...
| V12 | Recovery and commands serialize; Unknown gates mutations; repeated recovery preserves identity and unrelated completed evidence             |

## 6. Completion criteria

- [ ] P1–P6 and V1–V12 have passed all required verification.
...
`Node::open(NodeConfig)` uses real Git and the Ora local clock; the process composition root first
