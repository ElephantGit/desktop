...
工作流会话在首次创建时把节点选择直接持久化到 Session 行，并在恢复、重建、切换 Agent 和刷新时读取该值。恢复过程不再通过节点运行关联反推权限，因此关联缺失或成为孤儿数据时，也不会把显式选择扩大为自动发现。迁移时，普通历史会话保持自动发现，已绑定工作流的历史会话从冻结快照回填显式选择。修改草稿不会影响已有运行的选择。
