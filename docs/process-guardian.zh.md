...
新 host 日志版本为 3，新 guardian 日志版本为 4（增加 Run 日志），均不再有凭据列。Host 恢复事务化迁移精确 v1/v2 布局，
...
后接管、强停／关闭，以及 guardian SIGKILL 后工作负载仍活着但不持有 Scope 锁。

Controller 授权和租约已推迟。生产宿主 app、host 侧 Run 意图／投影、stdin、持久输出、
