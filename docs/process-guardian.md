...
Ready follows initialization; it proves no Run was started and no cleanup was completed. Launch errors,
...
five-second I/O deadline and channel workers are independently bounded. Current operations are Ready,
host binding and session inspection; workload execution is the next slice.

## Existing files and versions

New host and guardian journals use version 3 and contain no credential columns. Host recovery migrates
...
Full Controller authorization, leases, a production host app, workload IPC, stdin, durable output,
Node/Git/plugin integration and guardian-death recovery remain unfinished. Strong containment,
