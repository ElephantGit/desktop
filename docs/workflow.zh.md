...
`graph` 字段保存完整 React Flow JSON。编辑期间，工作流定义 CRUD 将其视为不透明字符串；发布前会校验稳定 ID 和图结构，运行启动时再由[工作流运行引擎](../crates/application/src/workflow_run/engine/README.md)校验可执行前置条件。
...
图中使用 `mcps: [{ mcpId, enabled }]` 保存绑定，其中 `mcpId` 是完整插件 ID。草稿保存、复制和导入导出均保留绑定，包括已禁用项。空列表或旧图缺少 `mcps` 时表示节点未授权任何 MCP。旧示例 ID 原样保留，不猜测替代插件；含非规范 ID 的导入只生成可编辑草稿，发布对话框会列出对应节点并阻止确认，用户移除或替换绑定后才能发布。图解析会拒绝空 ID、重复 ID、非规范 ID 和非法字段类型。
