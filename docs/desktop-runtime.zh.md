...
市场索引在启动十五秒后及每六小时刷新，按宿主本地时区调度，开发构建也启用。刷新按源独立进行：某个源失败时保留上一次索引中该源的条目，且不重扫它可能写了一半的检出，其余源照常刷新；只有被移除或禁用的源才真正丢失条目。失败随缓存记录并由 list/sync 返回，界面据此指明哪些源的列表陈旧，而不是把部分陈旧的目录当作刚刚同步。只有当至少一个源真正刷新成功时同步时间才前移，全部失败时保持原时间，从未成功过则显示「尚未同步」。失败等待下一周期，不自行重试。Backend 同时只接纳一次重建，重复请求返回缓存。自动同步事件使界面禁用 Sync，完成后使列表查询失效。
