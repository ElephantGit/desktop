...
//! to `<data-dir>/plugins/data/<namespace>/<name>/logs/plugin.log`. Host facts about the
//! plugin — lifecycle, protocol, and the health of this very pipeline — stay in the Ora runtime
//! log; the plugin's own statements never do.
...
pub use envelope::{MAX_IDENTIFIER_BYTES, PLUGIN_LOG_ENVELOPE_V1_PREFIX};
pub use pipeline::{MAX_RECORD_BYTES, PluginLogSetup, PluginLogStats, QUEUE_CAPACITY};
pub use record::{DEFAULT_PLUGIN_TARGET, RAW_STDERR_TARGET};
