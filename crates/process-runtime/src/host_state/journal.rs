...
pub(super) fn check_engine() -> Result<(), HostStateError> {
...
/// Opens an already created file and verifies the journal-specific WAL/FULL policy took effect.
pub(super) fn open_writable(path: &Path) -> Result<Connection, HostStateError> {
    ...
    let settings = connection.query_row(
        "SELECT (SELECT journal_mode FROM pragma_journal_mode), (SELECT synchronous FROM pragma_synchronous)",
...
pub(super) fn inspect(
...
) -> Result<HostBinding, HostStateError> {
    let connection = Connection::open_with_flags(
        path,
    ...
    if header != (APPLICATION_ID, VERSION) {
        return Err(HostStateError::Rejected(
    ...
    if definitions != [HOST_SCHEMA, INTENT_SCHEMA] {
        return Err(HostStateError::Rejected(
    ...
    if integrity != "ok" {
        return Err(HostStateError::Rejected("journal integrity check failed"));
    }
...
pub(super) fn advance_binding(
    ...
    if changed != 1 {
        return Err(HostStateError::Rejected("missing host binding"));
    }
    transaction.commit()?;
    Ok(())
}
...
fn decode_intent(row: &rusqlite::Row<'_>) -> Result<ScopeCreationIntent, HostStateError> {
    let phase: String = row.get(/*idx*/ 4)?;
    if phase != "intent_recorded" {
        return Err(HostStateError::Rejected("unknown guardian creation phase"));
    }
...
fn epoch_to_sql(epoch: NonZeroU64) -> Result<i64, HostStateError> {
    i64::try_from(epoch.get())
        .ok()
        .filter(|epoch| *epoch > 0)
        .ok_or(HostStateError::Rejected("host epoch outside journal range"))
}

/// Rejects corrupt or zero stored epochs instead of coercing them into a new incarnation.
fn epoch_from_sql(epoch: i64) -> Result<NonZeroU64, HostStateError> {
    u64::try_from(epoch)
        .ok()
        .and_then(NonZeroU64::new)
        .ok_or(HostStateError::Rejected("invalid stored host epoch"))
}
